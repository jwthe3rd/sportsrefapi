...
def read_ind_stats(league, team_name, year, name, pos, desired_stats):
    ...
    compare_names = name.split(',')
    compare_teams = team_name.split(',')
    print(compare_names)
    ...
    compare_done = []
    for val in range(len(compare_names)):
        compare_done.append(False)

    for i in range(1,1000):
        if league=='nfl':
            for ndx, name in enumerate(compare_names):
                if not(compare_done[ndx]):
                    try:
                        if i >= 11:
                            numb = i-1
                        else:
                        ...
                        if df_fant['Tm'][0] != compare_teams[ndx]:
                            raise ValueError
                        if df_fant['Pos'][0] != pos:
                            raise ValueError

                        compare_done[ndx]=True
                        df_ret.append(df_fant)
                        players.append(name)
                        teams.append(compare_teams[ndx])


                        
                    except HTTPError:
                        df_ret.append([pd.DataFrame({"Error":['HTTP error! No data found with the requested information!']})])
                        compare_done[ndx] = True
